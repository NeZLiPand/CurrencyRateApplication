...
//        
    ...
    public String parseMinfinCurrencyCodes() throws IOException {
        Document tablePage = Jsoup.parse(new URL("https://index.minfin.com.ua/ua/reference/currency/code/"), 30000);
        ...
        for (int iterator = 0; iterator < currencyCodesTable.size(); iterator++) {
            ...
            if (!(requestedLetterCode = elementTable.select("td[class*=mono]")
                                                    .text()).isEmpty() &&
                (!(requestedNumericalCode = elementTable.select("td[class=r0][align=center]")
                 ...
                 !(requestedNumericalCode = elementTable.select("td[class=r1][align=center]")
                                                        .text()).isEmpty()) &&
                (elementTable.select("td[class=r1][align=center]>acronym")
                             .text()).isEmpty() &&
                (elementTable.select("td[class=r0][align=center]>acronym")
                             .text()).isEmpty()) {
                currencyCodesParsed.appendElement("currencyCode");
                currencyCodesParsed.child(iteratorCodesParsed)
                                   .appendElement("columnLetterCode")
                                   .appendText(requestedLetterCode);
                currencyCodesParsed.child(iteratorCodesParsed++)
                                   .appendElement("columnNumericalCode")
                                   .appendText(requestedNumericalCode);
            }
        }
        return currencyCodesParsed.toString();
    }
